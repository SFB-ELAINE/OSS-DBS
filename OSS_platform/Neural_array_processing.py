...
class Neuron_array(object):
    ...
    def create_structured_array(self):
        ...
        gl_ind = 0
        for inx_angle in range(len(self.VTA_structure["X_angles_glob"])):           # d["alpha_array_glob"] - rotation around X, d["beta_array_glob"] - around Y. These angle lists should have the same length.
    ...
    def get_neuron_morhology(self,fib_diam,N_Ranvier):          # pass fib_diam and N_Ranvier explicitly, because they might be from a list
        ...
        if self.neuron_model == 'McIntyre2002': 
            ...
            print("The neuron model "+self.neuron_model+" is not implemented, exiting")
            raise SystemExit    
    ...
    def allocate_neurons_PO(self):    
        ...
        if self.Type == 'Custom':    # if manual placement
                ...
                for X_angle in self.custom_structure["X_angles_loc"]:
                    for Y_angle in self.custom_structure["Y_angles_loc"]:
                        for Z_angle in self.custom_structure["Z_angles_loc"]:                
                            ...
                            self.custom_coord_PO[loc_ind:loc_ind + self.pattern['num_segments'],:] = self.place_neuron(X_angle,Y_angle,Z_angle,X_locations_PO[inx],Y_locations_PO[inx],Z_locations_PO[inx])
                            loc_ind = loc_ind + self.pattern['num_segments']
                            
            np.savetxt('Neuron_model_arrays/All_neuron_models.csv', self.custom_coord_PO , delimiter=" ") 
            ...
            np.savetxt('Neuron_model_arrays/All_neuron_models.csv', self.imp_proc_coord_PO , delimiter=" ") 
    ...
    def build_neuron_models(self):
        ...
        if self.Type == 'VTA':          # for VTA
            ...
            self.VTA_seeds_PO[:,0] = self.VTA_seeds_O[:,0] + self.VTA_structure['Seed_coordinates'][0] - self.MRI_first_voxel[0] 
            self.VTA_seeds_PO[:,1] = self.VTA_seeds_O[:,1] + self.VTA_structure['Seed_coordinates'][1] - self.MRI_first_voxel[1] 
            self.VTA_seeds_PO[:,2] = self.VTA_seeds_O[:,2] + self.VTA_structure['Seed_coordinates'][2] - self.MRI_first_voxel[2]
    ...
    def process_external_array(self):
        ...
        if not isinstance(self.pattern['fiber_diameter'],list):                     # for simplicity, we will treat it as a list with a single entry
        ...
        if self.imp_name[-4:] == '.csv':      
            ...
            Array_coord_get = read_csv(self.imp_name, delimiter=' ', header=None)        #
        ...
        for i in lst:
            ...
            __, self.pattern['num_segments'][population_index], __ = self.get_neuron_morhology(self.pattern['fiber_diameter'][population_index],self.pattern['num_Ranvier'][population_index])         # add a consistency check
    ...
    def adjust_neuron_models(self,Domains,MRI_param):        
        ...
        for i in range(len(Domains.Encup_index)):
            subdomains_enc.array()[subdomains_assigned.array() == Domains.Encup_index[i]] = 1   #neuron models cannot be located in CSF
        
        if isinstance(Domains.Float_contacts,list): 
            for i in range(len(Domains.Float_contacts)):
                subdomains_enc.array()[subdomains_assigned.array() == Domains.Float_contacts[i]] = 1       #neuron models cannot be located in floating conductors
        else:   
        ...
        if self.Type != 'Imported':
            ...
            while inx < Array_coord.shape[0]:
                ...
                if (submesh_encup.bounding_box_tree().compute_first_entity_collision(pnt) < submesh_encup.num_cells()):        #this is a condition to check whether the point is inside encap. layer or floating conductor
                    ...
                    Array_coord,inx = self.mark_to_remove(Array_coord,inx)     # will also shift inx to the end of the neuron   
                else:    
            ...
            Array_coord=Array_coord[~np.all(Array_coord==-100000000.0,axis=1)]  #deletes all marked (in self.mark_to_remove with -100000000.0) enteries          
                ...
                if not(type(List_of_arrays[i]) is np.ndarray):          # check for empty
                    ...
                    while inx < Array_coord.shape[0]:
                        ...
                        if (submesh_encup.bounding_box_tree().compute_first_entity_collision(pnt)<mesh.num_cells()):        #this is a condition to check whether the point is inside encap. layer or floating conductor
                            points_encap += 1
                            Array_coord,inx = self.mark_to_remove(Array_coord,inx,nsegm = self.pattern['num_segments'][i])   # will also shift inx to the end of the neuron
                        else:    
                            if not(mesh.bounding_box_tree().compute_first_entity_collision(pnt)<mesh.num_cells()*100):        #if one point of the neural model is absent, the whole model is disabled
                                points_outside +=1
                                Array_coord,inx = self.mark_to_remove(Array_coord,inx,nsegm = self.pattern['num_segments'][i])
