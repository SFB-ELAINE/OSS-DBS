...
Vertex_1 = geompy.MakeVertex(0, 0, 0.1933)
Cylinder_1 = geompy.MakeCylinderRH(0.12057, 0.2)
Cylinder_2 = geompy.MakeCylinder(Vertex_1, OZ, 0.2, 27.5)
DBS_lead = geompy.MakeFuseList([Cylinder_1, Cylinder_2], True, True)

#Sphere_2 = geompy.MakeSphereR("indent_radius")
#Cut_2 = geompy.MakeCutList(DBS_lead, [Sphere_2], True)
#Fillet_1 = geompy.MakeFillet(Cut_2, "fillet_radius", geompy.ShapeType["EDGE"], [18])
#DBS_lead=Fillet_1

#Cut_2 = geompy.MakeCutList(DBS_lead, [Sphere_2], True)
#Cut_2 = geompy.MakeFuseList([DBS_lead, Sphere_2], True, True)
#Fillet_1 = geompy.MakeFillet(Cut_2, "fillet_radius", geompy.ShapeType["EDGE"], [18])
#DBS_lead=Fillet_1

Sphere_2 = geompy.MakeSphereR(0.12057)
#Cut_2 = geompy.MakeCutList(DBS_lead, [Sphere_2], True)
Fillet_2 = geompy.MakeFuseList([DBS_lead, Sphere_2], True, True)
Fillet_1=Fillet_2
Cut_2=Fillet_2
DBS_lead=Fillet_1

c1 = geompy.CreateGroup(Fillet_1, geompy.ShapeType["FACE"])
geompy.UnionIDs(c1, [22])
Contact_1 = geompy.MakeShell([c1])

Vertex_2 = geompy.MakeVertex(0, 0, -1*encap_thickness)
Cylinder_3 = geompy.MakeCylinder(Vertex_2, OZ, 0.12057+encap_thickness, 0.1933+encap_thickness)
Cylinder_4 = geompy.MakeCylinder(Vertex_1, OZ, 0.2+encap_thickness, 27.5)
Sphere_2_encap = geompy.MakeSphereR(0.12057+encap_thickness)
fuse_encap = geompy.MakeFuseList([Cylinder_3, Cylinder_4,Sphere_2_encap], True, True)
encap = geompy.MakeCutList(fuse_encap, [DBS_lead], True)

Sphere_1 = geompy.MakeSphereR(ROI_radial)
encap_outer_ROI = geompy.MakeCutList(encap, [Sphere_1], True)
...
#[Free_face_1_1,Free_face_1_2,Free_face_1_3,Free_face_1_4,Free_face_1_6,Contact_2] = geompy.SubShapes(Cut_1, [3, 10, 15, 20, 25, 30])  #when some space left from the bottom?
#Contact_2 = geompy.MakeFuseList([Contact_2], True, True)

Contact_2_circle = geompy.MakeCircle(Vertex_1, OZ, 0.2)
Contact_2 = geompy.MakePrismVecH(Contact_2_circle, OZ, "ground_length")

geompy.TranslateDXDYDZ(Sphere_1, 0, 0, 0.12057)
geompy.TranslateDXDYDZ(fuse_encap, 0, 0, 0.12057)
geompy.TranslateDXDYDZ(Contact_2, 0, 0, 0.12057)
geompy.TranslateDXDYDZ(Contact_1, 0, 0, 0.12057)
geompy.TranslateDXDYDZ(DBS_lead, 0, 0, 0.12057)
geompy.TranslateDXDYDZ(encap, 0, 0, 0.12057)
geompy.TranslateDXDYDZ(encap_outer_ROI, 0, 0, 0.12057)
geompy.TranslateDXDYDZ(encap_inner_ROI, 0, 0, 0.12057)

ROI = geompy.MakeCutList(Sphere_1, [fuse_encap], True)
Fuse_all = geompy.MakeFuseList([DBS_lead, encap_outer_ROI, encap_inner_ROI, ROI], True, True)
...
geompy.addToStudy( Fuse_all, 'Fuse_all' )


#Group_2 = geompy.CreateGroup(DBS_lead, geompy.ShapeType["FACE"])
#geompy.UnionIDs(Group_2, [3])
#Contact_1 = geompy.MakeFaceWires([Group_2], 1)
...
###################################################################################################################################################
  ...
  print(" cut fuse all scaled up with neuron \n")
...
if len(reference_surface) != len(Group_partition_surface): #+len(Group_partition_Multi_surface):
	print("Geometry-Surface error please check ROI diameter and DBS lead Position {},{}\n".format(len(reference_surface),len(Group_partition_surface)))
print('Group_partition_surface {}\n'.format(Group_partition_surface,'\n'))
