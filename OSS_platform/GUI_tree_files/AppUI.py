...
class MainWindow(Functionalities):
    ...
    def run_command(self):
        ...
        output=subprocess.run(['xterm','-hold','-e','docker', 'run', '--volume', dir_code_OSS_platform+':/opt/OSS-DBS','--volume',home_dir+self.path_to_patient+':/opt/Patient', '--cap-add=SYS_PTRACE', '-it', '--rm', 'custom_oss_platform', 'python3', 'Launcher_OSS_lite.py'], stdout=subprocess.PIPE,stderr=subprocess.STDOUT)#
