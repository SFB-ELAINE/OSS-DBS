...
def prerefine_ground(mesh,subdomains_assigned,boundaries,Domains):
    ...
    #print("zmin: ",zmin)
    ground_height=1000.0        #grounding on the whole exterior domain, make sure it is large enough
    ...
    for cell in cells(mesh): 
        ...
        if z_coord<zmin+ground_height and subdomains_just_here[cell]!=4 and subdomains_just_here[cell]!=5:      #mark all exterior cells excluding encap and floats
    ...
    file=File('Results_adaptive/grounding_cells.pvd')
    file<<cells_to_ref  
...
def refine_external_ground(Domains):        #if we want to skip adaptive mesh refinement
    ...
    if ref_it==1:   #if not ref was neccessary, just reload
        mesh = Mesh("Meshes/Mesh_unref.xml")
        boundaries = MeshFunction('size_t',mesh,'Meshes/Mesh_unref_facet_region.xml')
        subdomains_assigned=MeshFunction('size_t',mesh,"Meshes/Mesh_unref_physical_region.xml")    
        mesh,subdomains_assigned,boundaries,ground_surface_old=prerefine_ground(mesh,subdomains_assigned,boundaries,Domains)
    ...
    mesh_file=File('Results_adaptive/mesh_adapted_ground.xml.gz')
    boundaries_file = File('Results_adaptive/boundaries_adapted_ground.xml')
    subdomains_assigned_file=File('Results_adaptive/subdomains_assigned_adapted_ground.xml')
