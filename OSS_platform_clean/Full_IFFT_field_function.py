...
def get_field_in_time(d,FR_vector_signal,Xs_signal_norm,t_vector,N_models,n_segments):
...
#==============================Trunc1and2===================================#
    ...
    if d["VTA_from_NEURON"]==True:
        if not isinstance(N_models,list): 
            N_models=[N_models]
            n_segments=[n_segments]

        global_i=-1

        for i_model in range(len(N_models)):
           for loc_count in range(N_models[i_model]):
               global_i=global_i+n_segments[i_model]
               np.save('Axons_in_time/Signal_t_conv'+str(global_i), Signal_t_conv[global_i+1-n_segments[i_model]:global_i+1])    #save indexed as the last point (globally)
               #global_i=global_i+n_segments[i_model]
